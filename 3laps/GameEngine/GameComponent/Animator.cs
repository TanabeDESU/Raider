...
namespace GameEngine
{
    ...
    public class Animator:GameComponent
    {
        ...
        public void SetAnimation(string key)//再生するAnimationを設定する
        {
        ...
        public override void Update()//Animationの更新処理
        {
            count++;
            if(count > maxCount)
            {   
        ...
        {
            Animator animator = (Animator)MemberwiseClone();
            Dictionary<string, Animation> cloneAnimations = new Dictionary<string, Animation>();
            foreach(var b in animations)
            {
                cloneAnimations.Add(b.Key, b.Value.Clone(obj));
            }
            animator.animations = cloneAnimations;
            animator.nowAnimation = nowAnimation.Clone(obj);
            return animator;
        }
    }
}
